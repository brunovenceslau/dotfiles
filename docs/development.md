...
[ -s "$err_list" ] && cat "$err_list" >&2   # git warned (e.g. an unreadable dir) - treat as a failure
...
for f in sys.stdin.buffer.read().split(b"\0")[:-1]:
    ...
    assert real == root or real.startswith(root + os.sep), f"{f}: outside repo root"
    compile(open(f, "rb").read(), f, "exec")' < "$py_list"
rm -f "$py_list" "$err_list"
